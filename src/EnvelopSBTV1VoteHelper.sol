...
interface ISBTLight is IERC721Enumerable, IERC721Metadata {
    ...
    function wnftInfo(uint256 _tokenId) external view returns (ETypes.WNFT memory wnft);
}

contract EnvelopSBTV1VoteHelper {
    uint256 public constant DEFAULT_WRAPPED_MULTIPLIER = 3;
    address public immutable DEFAULT_WNFT_CONTRACT;
    address public immutable DEFAULT_ERC20_COLLATERAL;
    ...
    function getWrappedBalance(address _user, address _erc20, address _wnftAddress)
        external
        view
    ...
    function balanceOf(address _user) external view returns (uint256) {
        return IERC20Read(DEFAULT_ERC20_COLLATERAL).balanceOf(_user)
            + _getWrappedBalance(_user, DEFAULT_ERC20_COLLATERAL, DEFAULT_WNFT_CONTRACT) * DEFAULT_WRAPPED_MULTIPLIER;
    }

    function _getWrappedBalance(address _user, address _erc20, address _wnftAddress)
        internal
        view
        returns (uint256 wrappedBalance)
    {
